...
} from "@/src/lib/features/auth/authApi";
...
export const UserProfile: React.FC = () => {
  ...
  const [selectedBookingForDateChange, setSelectedBookingForDateChange] =
  ...
  const [changeBookingDate, { isLoading: isChangingDate }] =
    useChangeBookingDateMutation();
  ...
  const handleDateChangeSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setDateChangeError(
        err?.data?.error || "Failed to update visit date. Please try again.",
      );
    }
  };
  ...
  if (!isAuthenticated) {
    return (
      <div className="flex flex-col items-center justify-center py-20 min-h-[60vh]">
        ...
        <Link
          href="/login"
          className="px-8 py-3 bg-primary text-white rounded-xl font-bold shadow-lg hover:-translate-y-1 transition-transform"
        >
          {tNavigation("login")}
        </Link>
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="glassmorphism bg-white rounded-3xl p-8 md:p-12 shadow-ambient border border-outline-variant/10 flex flex-col md:flex-row items-center md:items-start gap-8 mb-12 relative overflow-hidden">
        ...
        <div
          className={`flex-1 z-10 flex flex-col justify-center h-full pt-2 ${isRtl ? "text-center md:text-right" : "text-center md:text-left"}`}
        >
          <h1 className="text-3xl md:text-5xl font-black text-secondary font-cairo mb-2 tracking-tight">
            {user?.name || "Dreamer"}
          </h1>
          <p className="text-on-surface/60 font-medium text-lg mb-6">
            {user?.email}
          </p>

          <div
            className={`flex flex-wrap gap-4 justify-center ${isRtl ? "md:justify-start" : "md:justify-start"}`}
          >
      ...
      <div className="mb-8 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex bg-surface p-1.5 rounded-2xl shadow-sm border border-outline-variant/10 relative">
          ...
          >
            ...
            <span
              className={`relative z-10 ${activeTab === "upcoming" ? "text-white" : "text-on-surface/60 hover:text-secondary"}`}
            >
          ...
          >
            ...
            <span
              className={`relative z-10 ${activeTab === "past" ? "text-white" : "text-on-surface/60 hover:text-secondary"}`}
            >
          ...
          >
            ...
            <span
              className={`relative z-10 ${activeTab === "cancelled" ? "text-white" : "text-on-surface/60 hover:text-secondary"}`}
            >
      ...
      {displayBookings.length === 0 ? (
        <div className="bg-surface-container-low rounded-3xl p-16 text-center border border-outline-variant/10">
          ...
          <p className="text-on-surface/60 max-w-md mx-auto mb-6">
            {tBooking("no_passes_desc", {
              tab: tBooking(activeTab + "_title").toLowerCase(),
            })}
          </p>
          <Link
            href="/tickets"
            className="inline-block px-8 py-3 bg-primary text-white rounded-xl font-bold shadow-lg shadow-primary/20 hover:-translate-y-1 transition-all"
          >
            {tBooking("book_pass")}
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {displayBookings.map((booking: any, index: number) => (
            <BookingQrCard
              ...
              onChangeDate={(id) =>
                handleOpenDateChange(id, booking.targetDate)
              }
            />
          ))}
        </div>
